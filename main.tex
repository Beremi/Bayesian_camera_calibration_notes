...
\begin{figure}[t]
\centering
\begin{tikzpicture}[font=\footnotesize, >=Latex, x=0.72cm, y=0.95cm]
  ...
  \foreach \x/\lbl in {3.2/k\!-\!1,6.5/k,10.6/k\!+\!1,13.0/k\!+\!2} {
    ...
    \draw[Accent!65] (\x,3.35) -- (\x,3.62);
    \node[font=\scriptsize] at (\x,4.45) {$t_{\lbl}$};
  }

  \node[reportsoft, text width=4.0cm] (topimu) at (2.8,5.15) {IMU packets arrive more frequently\\and only propagate the prior};
  ...
  \node[reportsoft, text width=3.2cm] (prior) at (5.9,0) {2. At exposure time $t_k$, extract the local pose / tag prior};
  \node[reportfactor, text width=3.2cm] (cam) at (10.1,0) {3. Run camera inversion on the current frame};
  \node[reportstate, text width=3.3cm] (msg) at (14.4,0) {4. Remove the local prior and assimilate the message};
  \node[reportmap, text width=5.1cm] (map) at (10.1,-2.9) {5. Birth new tags cautiously and gate reappearing tags\\inside the same joint state and covariance};

  \draw[reportarrow] (imu) -- (prior);
  \draw[reportarrow] (prior) -- (cam);
  \draw[reportarrow] (cam) -- (msg);
  \draw[reportarrow] (msg.south west) -- ++(-0.3,-0.55) -| (map.east);
