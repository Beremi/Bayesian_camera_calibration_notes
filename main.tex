...
The report is organized from modeling assumptions to practical estimators. A reader who knows basic linear algebra and probability can follow the main line without first knowing Lie theory or visual--inertial odometry.

\begin{enumerate}
  \item First read the frame convention and pose geometry. The symbols $T_{AB}$, $\Exp$, and $\Log$ are used everywhere else.
  \item Then read the Bayesian target: what is unknown, what is observed, and why a camera posterior is not the same thing as a fresh measurement.
  \item Next read the prior and likelihood sections. They explain which assumptions come from dynamics, which come from sensors, and which are engineering approximations.
  \item Finally read the inference and implementation sections. They turn the exact posterior into practical MAP, EKF/IEKF, message-passing, or fallback pose-summary algorithms.
\end{enumerate}

The notation tables are part of the exposition, not appendices. They are intended to keep each section self-contained.

\clearpage

\chapter{Foundations and Problem Setup}
\ChapterRoadmap{This chapter fixes frames and pose notation, states the Bayesian target, introduces the trajectory and map priors, writes the IMU and camera likelihoods, and only then derives the exact posterior and its numerical approximations. The order is deliberate: geometry first, probability model second, algorithms third.}
...
This convention fixes the camera--IMU extrinsic notation used later. The transform $T_{BC}$ is the pose of the camera frame $C$ expressed in the body/IMU frame $B$, so it maps camera coordinates into body coordinates. Its inverse $T_{CB}=T_{BC}^{-1}$ maps body coordinates into camera coordinates. Therefore
\[
T_{WC}=T_{WB}T_{BC},
\qquad
T_{WB}=T_{WC}T_{CB}.
...
A group is a set $G$ with a binary operation, written here as multiplication, satisfying four properties: closure ($g_1g_2\in G$ for $g_1,g_2\in G$), associativity ($(g_1g_2)g_3=g_1(g_2g_3)$), an identity element $e$ with $eg=ge=g$, and an inverse $g^{-1}$ for every element. A Lie group is a group that is also a smooth manifold, with multiplication and inversion varying smoothly with the group element. In this report the concrete Lie groups are $\SO$ for orientation and $\SE$ for full rigid pose. What matters in practice is that one can compose and invert poses globally, while still doing calculus and Gaussian approximation in a local vector space. That local vector space is the tangent space at the identity, also called the Lie algebra.

The group properties are concrete here, not abstract decoration. If $R_1,R_2\in\SO$, then
\begin{equation}
(R_1R_2)\tran(R_1R_2)=I,\qquad \det(R_1R_2)=\det(R_1)\det(R_2)=1,
\end{equation}
so $R_1R_2\in\SO$. The identity is $I$, the inverse is $R^{-1}=R\tran$, and associativity is inherited from matrix multiplication. The orthogonality and determinant constraints define a smooth three-dimensional manifold inside $\R^{3\times3}$, and matrix multiplication / transpose are smooth operations on it. Thus $\SO$ is a Lie group.

For poses, multiplication and inversion are
\begin{equation}
\begin{bmatrix}R_1&p_1\\0&1\end{bmatrix}
\begin{bmatrix}R_2&p_2\\0&1\end{bmatrix}
=
\begin{bmatrix}R_1R_2&p_1+R_1p_2\\0&1\end{bmatrix},
\qquad
\begin{bmatrix}R&p\\0&1\end{bmatrix}^{-1}
=
\begin{bmatrix}R\tran&-R\tran p\\0&1\end{bmatrix}.
\end{equation}
Closure follows because $R_1R_2\in\SO$ and $p_1+R_1p_2\in\R^3$; the identity is $I_4$; associativity again comes from matrix multiplication; and the inverse has the same rigid-transform form. Since $\SE$ is the smooth semidirect-product manifold $\SO\ltimes\R^3$ and these formulas are smooth in $(R,p)$, $\SE$ is also a Lie group.
...
\begin{equation}
\operatorname{expm}(A)=\sum_{n=0}^{\infty}\frac{A^n}{n!}.
\end{equation}
For a Lie group, this sends a Lie-algebra element to a group element. We use the standard robotics shorthand
\begin{equation}
\Exp([\phi]_\times)=\operatorname{expm}([\phi]_\times)\in\SO,\qquad
\Exp(\xi^\wedge)=\operatorname{expm}(\xi^\wedge)\in\SE,
\end{equation}
and
\begin{equation}
\Log(R)=\bigl(\operatorname{logm}(R)\bigr)^\vee\in\R^3,\qquad
\Log(T)=\bigl(\operatorname{logm}(T)\bigr)^\vee\in\R^6,
\end{equation}
where $\operatorname{logm}$ is a locally chosen matrix logarithm. The logarithm is local rather than globally unique: for rotations the branch is unambiguous for angles strictly below $\pi$, while an angle of $\pi$ has unavoidable axis/sign ambiguities. This is acceptable for filtering and Gauss--Newton updates because residuals are kept near the identity by relinearization.
...
\end{equation}
This is why $\R^3$ is used as the minimal local coordinate system for small rotation errors. With $\theta=\norm{\phi}$, the rotation exponential has the Rodrigues form
\begin{equation}
\Exp([\phi]_{\times})
=I+\frac{\sin\theta}{\theta}[\phi]_{\times}
 +\frac{1-\cos\theta}{\theta^2}[\phi]_{\times}^2,
\label{eq:rodrigues_exp_sweep}
\end{equation}
where the coefficients are interpreted by their Taylor limits when $\theta\to0$. Conversely, for $R\in\SO$ with rotation angle $\theta\in(0,\pi)$,
\begin{equation}
\Log(R)=\left(\frac{\theta}{2\sin\theta}(R-R\tran)\right)^\vee,
\qquad
\theta=\cos^{-1}\!\left(\frac{\tr(R)-1}{2}\right).
\label{eq:rotation_log_sweep}
\end{equation}
The small-angle limit of \eqref{eq:rotation_log_sweep} is simply $\Log(R)\approx \veeop(\tfrac12(R-R\tran))$. A small orientation perturbation $\delta\theta\in\R^3$ updates a nominal rotation by
\begin{equation}
...
\end{equation}
Its exponential can be written explicitly, with the same $\theta=\norm{\phi}$, as
\begin{equation}
\Exp(\xi^\wedge)=
\begin{bmatrix}
\Exp([\phi]_{\times}) & J(\phi)\rho\\
0 & 1
\end{bmatrix},
\qquad
J(\phi)=I+\frac{1-\cos\theta}{\theta^2}[\phi]_{\times}
 +\frac{\theta-\sin\theta}{\theta^3}[\phi]_{\times}^2,
\label{eq:se3_exp_sweep}
\end{equation}
again with Taylor limits at $\theta=0$. Thus the translational block of a pose increment is not merely $\rho$ when there is a finite rotation; it is coupled through the left Jacobian $J(\phi)$. Conversely, if
\begin{equation}
T=\begin{bmatrix}R&p\\0&1\end{bmatrix}\in\SE,
\end{equation}
then the local logarithm is obtained by
\begin{equation}
\phi=\Log(R),\qquad \rho=J(\phi)^{-1}p,\qquad \Log(T)=\begin{bmatrix}\rho\\\phi\end{bmatrix}.
\label{eq:se3_log_sweep}
\end{equation}
...
This event-driven viewpoint keeps the modeling honest while still allowing modular implementations. The camera inversion layer is free to use nonlinear optimization, robust losses, sliding windows, or visible-tag subsets internally. The main recursive estimator must distinguish a likelihood contribution from a posterior summary whenever the camera block used the predicted prior internally. The literature comparison is collected near the end of the report, after the model and approximation hierarchy have been defined.
...
Because these are continuous-time spectral densities, their discrete effect depends on the integration interval. For example, a velocity-drive covariance over a short interval is proportional to $Q_v\Delta t$, and a bias random-walk increment covariance is proportional to $Q_{bg}\Delta t$ or $Q_{ba}\Delta t$. This scaling is the reason that continuous-time noise densities should not be copied directly into a per-frame covariance without accounting for the sample interval.
...
\NotationTable{
$Q_v$ & Controls translational roughness. Large values allow larger acceleration-like changes in velocity.\\
$Q_\omega$ & Controls rotational roughness. Large values allow larger angular-acceleration-like changes in angular velocity.\\
...
\NotationTable{
$\mathcal U_k$ & All IMU packets in a chosen propagation interval, often between two camera exposure times $t_k$ and $t_{k+1}$.\\
...
\dot b_a &= w_{ba}.
\end{align}
A first-order discrete propagation over one short interval of length $\Delta t$ is
\begin{equation}
...
\end{equation}
The discrete noise terms above are not independent free parameters; they arise from integrating $n_g,n_a,w_{bg},w_{ba}$ over the actual propagation interval, and their joint covariance is summarized by $Q_k$. In an event-driven implementation the interval may be one IMU sample or the time from the previous event to the current event. If one writes a camera-to-camera transition, it is the accumulated result of all IMU packets between the two exposure times. Equation~\eqref{eq:discrete_process_model_final} is therefore a first-order discretization of the IMU-conditioned state transition used in EKF-, IEKF-, or fixed-lag estimators \citep{mourikis2007msckf,forster2017preintegration,barrau2017invariant}.
...
\begin{tikzpicture}[font=\footnotesize, >=Latex, x=1cm, y=1cm]
  \node[reportsoft, text width=3.9cm] (pred) at (0,1.25) {Predictive prior\\at exposure time\\$p^-_k(x_k,M)$};
  \node[reportsoft, text width=3.9cm] (pixels) at (0,-1.25) {Raw visual evidence\\tag corners, ids,\\scores $Y_k$};
  \node[reportbox, text width=4.4cm] (front) at (5.0,0) {Camera inversion layer\\local pose / visible-tag solve\\with robust pixel residuals};
  \node[reportsoft, text width=4.2cm] (msg) at (10.0,1.25) {Preferred output\\prior-free message $\lambda_k$\\or $(\Delta\Lambda_k,\Delta\eta_k)$};
  \node[reportsoft, text width=4.2cm] (pose) at (10.0,-1.25) {Fallback output\\pose summary only\\$z_k^v=(\widehat T_{WC,k},R_{v,k})$};
  \node[reportbox, text width=4.4cm] (est) at (15.2,0) {Event-driven estimator\\IMU prediction +\\camera assimilation\\on $\SO$ and $\SE$};
  \draw[reportarrow] (pred.east) -- ++(0.55,0) |- (front.north west);
  \draw[reportarrow] (pixels.east) -- ++(0.55,0) |- (front.south west);
  \draw[reportarrow] (front.north east) -- ++(0.65,0) |- node[pos=0.30, above, font=\scriptsize, fill=white, inner sep=1pt] {likelihood path} (msg.west);
  \draw[reportdash] (front.south east) -- ++(0.65,0) |- node[pos=0.30, below, font=\scriptsize, fill=white, inner sep=1pt] {reduced fallback} (pose.west);
  \draw[reportarrow] (msg.east) -- ++(0.65,0) |- (est.north west);
  \draw[reportdash] (pose.east) -- ++(0.65,0) |- (est.south west);
\end{tikzpicture}}
...
\ChapterRoadmap{This chapter translates the model into implementation choices. It starts with parameter roles and representative hardware scales, then specifies the event-driven architecture, initialization and tag bookkeeping, and finally lists approximation risks and validation statistics.}
...
\resizebox{0.98\textwidth}{!}{%
\begin{tikzpicture}[font=\footnotesize, >=Latex, x=1cm, y=1cm]
  \node[font=\scriptsize\itshape, text=Accent] at (3.9,2.65) {event-time prediction lane};
  \node[font=\scriptsize\itshape, text=Accent] at (9.2,2.65) {camera-frame likelihood lane};
  \node[font=\scriptsize\itshape, text=Accent] at (9.2,-2.75) {persistent map lane};
  \node[reportsoft, text width=3.1cm] (events) at (0,0.35) {Time-ordered event queue\\IMU packets and camera frames};
  \node[reportbox, text width=4.1cm] (pred) at (4.3,0.35) {Joint predictor\\propagate nominal state and covariance\\to current event time};
  \node[reportsoft, text width=3.8cm] (priorbundle) at (8.7,1.45) {Local prior bundle\\current pose prior +\\visible mapped-tag priors};
  \node[reportbox, text width=3.8cm] (cam) at (13.0,1.45) {Camera inversion\\pixels + local priors\\$\mapsto$ prior-free message};
  \node[reportbox, text width=4.1cm] (upd) at (13.0,-1.25) {Joint estimator\\message assimilation, tag birth,\\and reappearance gating};
  \node[reportsoft, text width=3.0cm] (memory) at (8.7,-1.25) {Persistent tag posteriors\\and cross-covariances};
  \node[reportsoft, text width=2.8cm] (anchor) at (4.3,-2.75) {Known anchor\\$T_{WT_0}$};
  \coordinate (statepathleft) at (6.65,-2.15);
  \coordinate (statepathright) at (10.85,-2.15);
  \coordinate (updstatein) at ($(upd.west)+(0,-0.55)$);
  \draw[reportarrow] (events.east) -- (pred.west);
  \draw[reportarrow] (pred.east) -- ++(0.45,0) |- node[pos=0.56, left, font=\scriptsize, fill=white, inner sep=1pt] {pose prior} (priorbundle.west);
  \draw[reportarrow] (priorbundle.east) -- (cam.west);
  \draw[reportarrow] (pred.south east) -- ++(0.35,0) |- (statepathleft) -- node[midway, below, font=\scriptsize, fill=white, inner sep=1pt] {propagated joint state} (statepathright) |- (updstatein);
  ...
  \draw[reportarrow] (upd.west) -- (memory.east);
  \draw[reportdash] (memory.north) -- node[midway, right, font=\scriptsize, fill=white, inner sep=1pt] {stored tag priors} (priorbundle.south);
\end{tikzpicture}}
...
\[
p(X_{0:N},M\mid \mathcal U_{0:N-1},Y_{0:N})
\propto
p_0(x_0)p_0(M)
\prod_{k=0}^{N-1} p(x_{k+1}\mid x_k,\mathcal U_k)
...
This design is still approximate, because the camera inversion itself is local and usually nonlinear, and because only a subset of the full joint posterior is exposed to the camera block at each frame. It is nevertheless more faithful to the Bayesian factorization than a pose-summary-only pipeline. A clean implementation should obey the following rules.
\begin{itemize}
...
\begin{tikzpicture}[font=\footnotesize, >=Latex, x=1cm, y=1cm]
  ...
  \tikzstyle{camevt}=[evt,fill=orange!14,thick]
  \tikzstyle{oldcamevt}=[evt,fill=orange!6,draw=Accent!55]

  \draw[reportarrow] (0,3.35) -- (16.4,3.35) node[right] {time};
  \foreach \x in {0.8,1.6,2.4,3.8,4.6,5.4,6.2,7.0,8.6,9.4,10.2,11.0,12.4,13.2,14.0,14.8} {
    \node[imuevt] at (\x,3.9) {\scriptsize I};
    \draw[Accent!65] (\x,3.35) -- (\x,3.62);
  }
  \foreach \x/\lbl in {3.0/k\!-\!1,11.6/k\!+\!1,15.2/k\!+\!2} {
    \node[oldcamevt] at (\x,3.9) {\scriptsize C};
    \draw[Accent!65] (\x,3.35) -- (\x,3.62);
    \node[font=\scriptsize, anchor=north west] at (\x,3.08) {$t_{\lbl}$};
  }
  \node[camevt] (camtk) at (7.8,3.9) {\scriptsize C};
  \draw[Accent!80, thick] (7.8,3.35) -- (7.8,3.62);
  \node[font=\scriptsize, anchor=north] at (7.8,3.05) {$t_k$};

  \node[reportsoft, text width=4.0cm] (topimu) at (2.6,5.15) {IMU packets arrive frequently\\and only propagate the prior};
  \node[reportsoft, text width=4.8cm] (topcam) at (7.8,5.35) {Camera exposure $t_k$\\triggers one visual likelihood contribution};
  \draw[reportarrow] (topimu.south) -- (2.4,4.2);
  \draw[reportarrow] (topcam.south) -- (camtk.north);

  \node[reportfactor, text width=3.0cm] (imu) at (1.6,0) {1. Propagate through arriving IMU events};
  \node[reportsoft, text width=3.45cm] (prior) at (7.8,0) {2. At exposure time $t_k$, extract the local pose / tag prior};
  \node[reportfactor, text width=3.25cm] (cam) at (11.5,0) {3. Run camera inversion on the current frame};
  \node[reportstate, text width=3.55cm] (msg) at (15.5,0) {4. Remove the local prior and assimilate the message};
  \node[reportmap, text width=5.4cm] (map) at (11.5,-2.55) {5. Birth new tags cautiously and gate reappearing tags\\inside the same joint state and covariance};

  \draw[reportarrow] (imu) -- (prior);
  \draw[reportarrow] (prior) -- (cam);
  \draw[reportarrow] (cam) -- (msg);
  \draw[reportarrow] (msg.south) -- ++(0,-0.65) -| (map.east);
  \draw[reportdash] (7.8,3.62) -- (prior.north);
  \node[font=\scriptsize, fill=white, inner sep=1pt] at (7.8,1.75) {advance exactly to $t_k$};
\end{tikzpicture}}
...
\]
Then convert to body pose using the known camera--IMU extrinsic, with $T_{CB}=T_{BC}^{-1}$,
\[
...
Several implementation shortcuts are acceptable approximations. Several others break the intended posterior semantics and should be treated as modeling errors.
\begin{itemize}
...
\subsection{Assessment of the modular message scheme}
The modular scheme is defensible when it is described as an approximation to the joint posterior, not as the joint posterior itself. The recommended reduced architecture is the event-driven message architecture described above. The camera code can stay separate, the back-end can remain recursive and real-time, and a slower map-refinement layer can still be added later. The key requirement is that a prior-conditioned camera posterior must not be treated as an independent sensor measurement unless the prior contribution has first been removed.

The direct raw-residual joint filter remains formally cleaner whenever it is computationally feasible. The pose-summary fallback remains usable for a first prototype or ablation study, but it should be documented as a weaker approximation whose neglected couplings and prior-reuse hazards can make the filter overconfident. This places the practical hierarchy in a clear order: exact joint posterior first, message-based recursive approximation second, pose-summary shortcut last \citep{mourikis2007msckf,leutenegger2015okvis,forster2017preintegration,kaess2012isam2,TagSLAM2019}.
...
\bottomrule
\end{tabularx}
\caption{Representative Bayesian formulations for trajectory-pose estimation. The same inverse problem can be expressed through path-space priors, discrete Markov priors, MAP objectives, or continuous-time Gaussian-process priors. The table is a positioning aid rather than an exhaustive survey.}
\end{table}

\clearpage
\appendix
\chapter{Prior samples for interpretation and sanity checking}
\ChapterRoadmap{The appendix visualizes the trajectory prior before any measurements are used. It is a sanity check for the chosen motion and bias hyperparameters, not a posterior result and not a claim about one unique physical experiment.}
...
The parameters in this appendix are discrete sampler settings. In particular, $\sigma_a$ and $\sigma_\alpha$ are the standard deviations of the acceleration-like samples drawn at each simulation step, and $\sigma_{bg,\mathrm{rw}}$ and $\sigma_{ba,\mathrm{rw}}$ are per-step bias increments. They should not be read as the continuous-time spectral-density matrices $Q_v,Q_\omega,Q_{bg},Q_{ba}$ without applying the appropriate $\Delta t$ scaling.
