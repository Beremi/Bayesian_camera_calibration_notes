...
\begin{figure}[H]
\centering
\begin{tikzpicture}[font=\footnotesize, >=Latex, x=0.88cm, y=1cm]
  \node[reportsoft, text width=2.7cm] (events) at (0,0) {Time-ordered event queue\\IMU packets and camera frames};
  \node[reportbox, text width=3.9cm] (pred) at (5.2,0) {Joint predictor\\propagate nominal state and covariance\\to the current event time};
  \node[reportbox, text width=3.8cm] (cam) at (10.1,1.5) {Camera inversion\\pixels + local pose/tag prior\\$\mapsto$ prior-free message};
  \node[reportbox, text width=3.8cm] (upd) at (10.1,-1.7) {Joint estimator and map state\\message assimilation, tag birth,\\and reappearance gating};
  \node[reportsoft, text width=2.7cm] (anchor) at (5.2,-3.6) {Known anchor\\$T_{WT_0}$};
  \node[reportsoft, text width=3.1cm] (memory) at (14.7,0) {Persistent tag posteriors\\and cross-covariances};
  \draw[reportarrow] (events) -- (pred);
  \draw[reportarrow] (pred.east) to[out=8,in=180] node[midway, above, font=\scriptsize, fill=white, inner sep=1pt] {local predictive prior} (cam.west);
  \draw[reportarrow] (pred.east) to[out=-10,in=180] (upd.west);
  \draw[reportarrow] (anchor.north) -- (pred.south);
  \draw[reportarrow] (cam.south) -- node[right, font=\scriptsize, fill=white, inner sep=1pt] {$\lambda_k$} (upd.north);
  \draw[reportarrow] (upd.east) -- (memory.west);
  \draw[reportdash, rounded corners=4pt] (memory.south) -- ++(0,-1.5) -| ([xshift=-1pt]cam.east);
\end{tikzpicture}
